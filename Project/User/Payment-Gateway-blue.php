...
$updateqry2="UPDATE tbl_house SET house_status = 3 where house_id='".$houseid."'";
$insertqry="INSERT INTO tbl_renter(user_id,house_id,renter_date)VALUES('".$userid."','".$houseid."',curdate())";
if($conn->query($updateqry)&&$conn->query($updateqry2)&&$conn->query($insertqry))
  {
	  ?>
        <script>
    alert("Payment Completed Succesfully");
    window.location="RentHouses.php";
    </script>
        <?php
  }else{
	  ?>
